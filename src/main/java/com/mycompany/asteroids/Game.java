...
public class Game extends JFrame {
	...
	private void updateGame() {
		...
		if(!isGameOver && areEnemiesDead()) {
			...
			//Agregar UNA nave alienígena por nivel
			registerEntity(new AlienShip(random));
		}
	...
	/**
	 * Determines whether or not any asteroids or alien ships still exist in the world.
	 * @return Whether or not all of the enemies are dead.
	 */
	private boolean areEnemiesDead() {
		for(Entity e : entities) {
			if(e.getClass() == Asteroid.class || e.getClass() == AlienShip.class) {
				return false;
			}
		}
		return true;
